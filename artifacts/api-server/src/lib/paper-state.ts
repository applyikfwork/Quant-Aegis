// Shared paper trading state — single source of truth for the entire application.
// Dashboard, Portfolio, Risk, and Trade Journal all read from here in offline mode.

import { getLivePrice as getMarketPrice } from "./live-prices";
...
export const SYMBOLS = ["BTCUSDT","ETHUSDT","SOLUSDT","BNBUSDT","XRPUSDT","AVAXUSDT","ADAUSDT","DOGEUSDT"];
...
export interface Order {
  ...
  status: "created"|"pending"|"validated"|"queued"|"submitted"|"partial"|"filled"|"cancelled"|"rejected"|"expired"|"archived";
...
// ─── PRICE ORACLE ─────────────────────────────────────────────────────────────
// Small random walk on top of real live prices — produces realistic tick movement
const priceOffsets: Record<string, number> = {};

export function getLivePrice(symbol: string): number {
  const base = getMarketPrice(symbol);
  if (!priceOffsets[symbol]) priceOffsets[symbol] = 0;
  priceOffsets[symbol] += (Math.random() - 0.5) * base * 0.0004;
  priceOffsets[symbol] = Math.max(-base * 0.005, Math.min(base * 0.005, priceOffsets[symbol]));
...
export let account: Account = {
  ...
  todayPnl: 0, weekPnl: -890.23, monthPnl: -3_569.78,
...
// Seed positions using live-prices-relative entry prices
export let positions: Position[] = [
  {
    id: "pos-001", symbol: "BTCUSDT", side: "long", quantity: 0.08,
    entryPrice: 61_800, currentPrice: 63000,
    unrealizedPnl: 0, unrealizedPnlPct: 0,
    stopLoss: 59_800, takeProfit: 67_000, trailingStop: null,
    leverage: 5, margin: 988.8, liquidationPrice: 49_440,
    strategy: "SMC Breakout", signalId: "sig-001", orderType: "market",
    openTime: new Date(Date.now() - 18 * 3600000).toISOString(),
    maxProfit: 180, maxDrawdown: -64, currentRisk: 1.8, atr: 950,
    status: "open",
    fills: [
      { qty: 0.05, price: 61790, ts: new Date(Date.now() - 18*3600000).toISOString(), fee: 1.85 },
      { qty: 0.03, price: 61815, ts: new Date(Date.now() - 18*3600000 + 800).toISOString(), fee: 1.11 },
    ],
    fee: 2.96, slippage: 8.4, notes: "AI breakout signal — SMC OB confirmed",
  },
  {
    id: "pos-002", symbol: "ETHUSDT", side: "long", quantity: 1.2,
    entryPrice: 1_640, currentPrice: 1700,
    unrealizedPnl: 0, unrealizedPnlPct: 0,
    stopLoss: 1_570, takeProfit: 1_840, trailingStop: null,
    leverage: 3, margin: 656, liquidationPrice: 1_313,
    strategy: "Trend Following", signalId: "sig-002", orderType: "limit",
    openTime: new Date(Date.now() - 42 * 3600000).toISOString(),
    maxProfit: 88, maxDrawdown: -32, currentRisk: 1.5, atr: 42,
    status: "open",
    fills: [
      { qty: 1.2, price: 1640, ts: new Date(Date.now() - 42*3600000).toISOString(), fee: 1.18 },
    ],
    fee: 1.18, slippage: 2.1, notes: "EMA breakout — volume confirmed",
  },
  {
    id: "pos-003", symbol: "SOLUSDT", side: "short", quantity: 8,
    entryPrice: 156, currentPrice: 150,
    unrealizedPnl: 0, unrealizedPnlPct: 0,
    stopLoss: 162, takeProfit: 138, trailingStop: null,
    leverage: 2, margin: 624, liquidationPrice: 234,
    strategy: "Mean Reversion", signalId: "sig-003", orderType: "market",
    openTime: new Date(Date.now() - 6 * 3600000).toISOString(),
    maxProfit: 36, maxDrawdown: -14, currentRisk: 1.2, atr: 3.2,
    status: "open",
    fills: [
      { qty: 8, price: 156, ts: new Date(Date.now() - 6*3600000).toISOString(), fee: 0.75 },
    ],
    fee: 0.75, slippage: 1.2, notes: "Overbought reversal — bearish divergence",
  },
];
...
function seedClosedTrades(): ClosedTrade[] {
  ...
  // Seed symbol base prices consistent with defaults in live-prices.ts
  const SEED_BASE: Record<string, number> = {
    BTCUSDT: 63000, ETHUSDT: 1700, SOLUSDT: 150, BNBUSDT: 576,
    XRPUSDT: 0.52, AVAXUSDT: 28, ADAUSDT: 0.44, DOGEUSDT: 0.12,
  };
  for (let i = 0; i < 44; i++) {
    const symbol = pick(SYMBOLS);
    const base = SEED_BASE[symbol] ?? 1;
    ...
    trades.push({
      id: `trade-${String(i + 1).padStart(3, "0")}`, symbol, side,
...
function seedOrders(): Order[] {
  ...
  for (let i = 0; i < 30; i++) {
    ...
    orders.push({
      id: `ord-${String(i + 1).padStart(3, "0")}`, positionId: status === "filled" ? `pos-${String(i + 1).padStart(3, "0")}` : null,
      ...
      validationLog: [
        "✓ Symbol validated", "✓ Quantity within limits", "✓ Sufficient margin",
        "✓ Risk check passed", "✓ Position limit OK",
        status === "rejected" ? "✗ Insufficient margin for requested leverage" : "✓ Market open",
        "✓ User permissions OK",
      ],
...
export let closedTrades: ClosedTrade[] = seedClosedTrades();
export let orders: Order[] = seedOrders();

// ─── COMPUTATION FUNCTIONS ────────────────────────────────────────────────────
export function computePositions(): (Position & { currentPrice: number; unrealizedPnl: number; unrealizedPnlPct: number })[] {
  return positions.map((p) => {
    ...
    return {
      ...p, currentPrice: Math.round(current * 100) / 100,
...
export function computeAccount() {
  ...
  // Compute today's P&L from closed trades today
...
export function computePerformance() {
  ...
  let peak = INITIAL_BALANCE; let maxDD = 0; let running = INITIAL_BALANCE;
  for (const t of all) {
    running += t.pnl;
  ...
  for (const t of all) {
    ...
    bySymbol[t.symbol].trades++; if (t.pnl > 0) bySymbol[t.symbol].wins++; bySymbol[t.symbol].pnl += t.pnl;
    if (!byStrategy[t.strategy]) byStrategy[t.strategy] = { trades: 0, wins: 0, pnl: 0 };
    byStrategy[t.strategy].trades++; if (t.pnl > 0) byStrategy[t.strategy].wins++; byStrategy[t.strategy].pnl += t.pnl;
  }
  const bestTrade = all.length > 0 ? Math.max(...all.map(t => t.pnl)) : 0;
  const worstTrade = all.length > 0 ? Math.min(...all.map(t => t.pnl)) : 0;
  ...
  return {
    ...
    bySymbol, byStrategy, bestTrade, worstTrade,
    pnlHistory: closedTrades.slice(-20).map((t, i, arr) => {
      const cumPnl = arr.slice(0, i + 1).reduce((s, x) => s + x.pnl, 0);
      return { date: t.closeTime.slice(0, 10), pnl: t.pnl, cumPnl: Math.round(cumPnl * 100) / 100 };
    }),
  };
}

// ─── EXECUTION ENGINE ─────────────────────────────────────────────────────────
export function simulateExecution(body: {
  ...
  if (!currentPrice || currentPrice <= 0) return { order: null as unknown as Order, error: "Invalid symbol" };
  ...
  if (margin > acc.freeMargin * 0.95) return { order: null as unknown as Order, error: "Insufficient free margin" };
  validationLog.push("✓ Margin check passed", "✓ Risk check passed", "✓ Position limit OK", "✓ Market open", "✓ User permissions OK", "✓ AI approval granted");
...
export function closePosition(id: string, exitReason = "manual"): { trade: ClosedTrade; error?: string } {
  const idx = positions.findIndex((p) => p.id === id);
  if (idx === -1) return { trade: null as unknown as ClosedTrade, error: "Position not found" };
...
export function resetAccount(): void {
  ...
  account = {
    ...
    todayPnl: 0, weekPnl: 0, monthPnl: 0, totalDeposits: INITIAL_BALANCE,
    resetCount: prev.resetCount + 1, createdAt: prev.createdAt,
  ...
  orders = seedOrders();
  closedTrades = [];
  Object.keys(priceOffsets).forEach(k => delete priceOffsets[k]);
}

// ─── GETTERS (for use by other routes) ────────────────────────────────────────
